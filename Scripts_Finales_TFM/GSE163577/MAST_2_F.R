...
for (i in 1:length(cell_types)){
  ...
  # Test hypothesis
  zlmCond_cells <- lrTest(zlmCond, Hypothesis("Group_sexAD_F - Group_sexControl_F"))
  ...
  # Calculate LFCs
  LFC.sF <- LFC.standard(data = sce.dge, results = results, case = "AD_F", control = "Control_F")
  
  LFC.mF <- LFC.MAST(zlm = zlmCond, control = "Group_sexControl_F", case = "Group_sexAD_F", results = results)
