...
plot <- ggplot(sce.filt.df, aes(x = Patient_ID, y = total_filt_counts, fill = Group_sex)) +
...
# After normalization #
plot <- ggplot(sce.norm.df, aes(x = Patient_ID, y = total_norm_counts, fill = Group_sex)) +
    geom_boxplot(outlier.size = 0.5, position = position_dodge()) +
