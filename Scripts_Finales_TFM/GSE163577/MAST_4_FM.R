...
# Esta función está cogida del script que me ha dado Fer, gracias Fer :'D
...
for (i in 1:length(cell_types)){
  ...
  # Test hypothesis
  zlmCond_cells_FM <- lrTest(zlmCond, Hypothesis("(Group_sexAD_F - Group_sexControl_F) -
                                                 (Group_sexAD_M - Group_sexControl_M)"))
  ...
  LFC.sF <- LFC.standard(data = sce.dge, results = resultsF, case = "AD_F", control = "Control_F")
  ...
  # Calculate LFCs
  LFC.sM <- LFC.standard(data = sce.dge, results = resultsM, case = "AD_M", control = "Control_M")
  
  LFC.mFM <- LFC.MAST(zlm = zlmCond,
                     control = c("Group_sexControl_F","Group_sexAD_M"),
                     case =  c("Group_sexControl_M","Group_sexAD_F"),
                     results = results_2)
