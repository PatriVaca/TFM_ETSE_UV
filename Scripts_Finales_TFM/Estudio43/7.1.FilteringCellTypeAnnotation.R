...
# 7.FilteringCelLTypeAnnotation_GV
...
system("mkdir -p figures/7.FilteredCellTypeAnnotation/7.1.ClusteringGV/prefiltering/")
system("mkdir -p figures/7.FilteredCellTypeAnnotation/7.1.ClusteringGV/filtered/")

scegv_clusters <- readRDS("processed_data/scegv_clusters_annot3.rds")

# ---------------------------- Prefiltering ----------------------------------

# CONSENSUAL RESULTS GENERATION #
path1 <- "figures/7.FilteredCellTypeAnnotation/7.1.ClusteringGV/prefiltering/"

# Convert intermedium nodes into unknowns to obtain the 3 annotation methods consensum
scegv_clusters$CHETAH_label <- str_replace(scegv_clusters$CHETAH_label, "Node1", "unknown")
scegv_clusters$CHETAH_label <- str_replace(scegv_clusters$CHETAH_label, "Node2", "unknown")
scegv_clusters$CHETAH_label <- str_replace(scegv_clusters$CHETAH_label, "Node3", "unknown")
scegv_clusters$CHETAH_label <- str_replace(scegv_clusters$CHETAH_label, "Node4", "unknown")
scegv_clusters$CHETAH_label <- str_replace(scegv_clusters$CHETAH_label, "Unassigned", "unknown")
scegv_clusters$SCINA_label <- str_replace(scegv_clusters$SCINA_label, "endotelio", "unknown")
scegv_clusters$SCINA_label <- str_replace(scegv_clusters$SCINA_label, "fibroblasto", "unknown")

# Combine the 3 annotation methods results
all_res <-data.frame(
  scegv_clusters$SCINA_label,
  scegv_clusters$CHETAH_label,
  scegv_clusters$singleR_label
)
...
write.table(all_res,file=paste0("processed_data/matrix_annot_gv.tsv"), sep="\t", row.names = FALSE)

# Annotated SCE creation
sce.annot  <- scegv_clusters
scegv_clusters$tipo.celular  <- all_res$cell_type
sce.annot$tipo.celular  <- all_res$cell_type
saveRDS(scegv_clusters, file  <- "processed_data/scegv_clusters_filtered_annot.rds")
...
path2 <- "figures/7.FilteredCellTypeAnnotation/7.1.ClusteringGV/filtered/"
...
saveRDS(sce.annot, file = "processed_data/scegv_clusters_filtered_annot2.rds")
...
require(openxlsx)
write.xlsx(proporciones, file = paste0("processed_data/abundancia_celular_gv.xlsx"),rowNames=TRUE)
