...
discard_count <- (sce.qc$sum < 400 | sce.qc$sum > 8000)
sce.qc$discard_count <- discard_count
print("Células eliminadas por tamaño libreria")
table(discard_count)
...
# Discard the cells
discard <-  (discard_count | discard_ngenes | discard_mito | discard_doublet)
...
# Discard genes not detected in X cells (10 in this case, as done in the original paper)
...
table(not_selected_genes | selected_features == FALSE)

# Genes to keep
nkeep <- !not_selected_genes & selected_features
...
plot <- scater::plotColData(sce.qc, x="Sample", y="sum",
                            colour_by="discard_count", 
                            other_fields = "Note") + 
  facet_wrap(~Note,
             scales = "free_x",
             labeller = labeller(Note=c("Alzheimer's Disease" = "AD",
                                       "Control" = "Control"))) +
...
ggsave(filename=paste0(path, "ComparativeCounts.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path, "ComparativeCounts.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")
 

plot <- scater::plotColData(sce.qc, x="Sample", y="detected",
...
plot <- scater::plotColData(sce.qc, x="Sample", y="subsets_Mito_percent",
                            colour_by="discard_mito", other_fields = "Note") +
  facet_wrap(~Note,
             scales = "free_x",
             labeller = labeller(Note=c("Alzheimer's Disease" = "AD",
                                       "Control" = "Control"))) +
...
plot <- scater::plotColData(sce.qc, x="Sample", y="sum",
    ...
    facet_wrap(~Note,
               ...
               labeller=labeller(Note=c("Alzheimer's Disease" = "AD",
                                        "Control" = "Control"))) +
    labs(title = "Conteos por paciente antes del filtrado", y = "Conteos por célula", x = "Paciente") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(hjust = 0.5)) +
    scale_color_viridis_d(name="Condición y sexo", labels=c("Alzheimer's Disease_Female"="AD_Mujer",
...
#------------------------- After QC plots --------------------------------
# To plot each cell discarding criteria
sce.filtsum <- sce.qc[nkeep, !discard_count]
sce.filtdetected <- sce.qc[nkeep, !discard_ngenes]
sce.filtmito <- sce.qc[nkeep, !discard_mito]

plot <- scater::plotColData(sce.filtsum, x="Sample", y="sum",
                            colour_by = "Group_sex",
                            other_fields = "Note") +
  facet_wrap(~Note,
             scales = "free_x",
             labeller = labeller(Note=c("Alzheimer's Disease" = "AD",
                                        "Control" = "Control"))) +
  labs(title = "Conteos por paciente tras el filtrado", y = "Conteos por célula", x = "Paciente") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(hjust = 0.5))+
  scale_color_viridis_d(name="Condición y sexo", labels=c("Alzheimer's Disease_Female"="AD_Mujer",
                                                          "Alzheimer's Disease_Male"="AD_Hombre",
                                                          "Control_Female"="Control_Mujer",
                                                          "Control_Male"="Control_Hombre"))
ggsave(filename=paste0(path, "AfterQC_Counts.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path, "AfterQC_Counts.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")


plot <- scater::plotColData(sce.filtdetected, x="Sample", y="detected",
                            colour_by = "Group_sex", other_fields = "Note") +
  facet_wrap(~Note,
             scales = "free_x",
             labeller = labeller(Note=c("Alzheimer's Disease" = "AD",
                                        "Control" = "Control"))) +
...
plot <- scater::plotColData(sce.filtmito, x="Sample", y="subsets_Mito_percent",
                            colour_by = "Group_sex",
                            other_fields = "Note") +
  facet_wrap(~Note,
             scales = "free_x",
             labeller = labeller(Note=c("Alzheimer's Disease" = "AD",
                                        "Control" = "Control"))) +
  labs(title = "% Mito por paciente tras el filtrado", y = "% Mitocondrial", x = "Paciente") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(hjust = 0.5))+
  scale_color_viridis_d(name="Condición y sexo", labels=c("Alzheimer's Disease_Female"="AD_Mujer",
                                                          "Alzheimer's Disease_Male"="AD_Hombre",
                                                          "Control_Female"="Control_Mujer",
                                                          "Control_Male"="Control_Hombre"))
ggsave(filename=paste0(path, "AfterQC_Mitochondrial.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path, "AfterQC_Mitochondrial.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")
...
df <- data.frame(Counts = c(counts_preQC, counts_postQC),
...
plot <- ggplot(df, aes(x = Counts, fill = QC_Status)) +
  ...
  labs( title = "Distribución de conteos antes y después del QC",
...
grouped_df <- coldata.qc %>%
  group_by(Sample, Group_sex) %>%
  summarise(Num_cels_discarded = sum(discard))

plot <- ggplot(grouped_df, aes(x = as.factor(Sample), y = Num_cels_discarded)) +
...
plot <- ggplot(grouped_df2, aes(x = Sample, y = Cantidad)) +
