...
# 7.FilteringCellTypeAnnotationPoisson
...
system("mkdir -p figures/7.FilteredCellTypeAnnotation/7.3.ClusteringPoisson/prefiltering")
system("mkdir -p figures/7.FilteredCellTypeAnnotation/7.3.ClusteringPoisson/filtered")

sceP_clusters <- readRDS("processed_data/sceP_clusters_annot3.rds")

# ---------------------------- Prefiltering ----------------------------------

# CONSENSUAL RESULTS GENERATION #
path1 <- "figures/7.FilteredCellTypeAnnotation/7.3.ClusteringPoisson/prefiltering/"

# Convert intermedium nodes into unknowns to obtain the 3 annotation methods consensum
sceP_clusters$CHETAH_label <- str_replace(sceP_clusters$CHETAH_label, "Node1", "unknown")
sceP_clusters$CHETAH_label <- str_replace(sceP_clusters$CHETAH_label, "Node2", "unknown")
sceP_clusters$CHETAH_label <- str_replace(sceP_clusters$CHETAH_label, "Node3", "unknown")
sceP_clusters$CHETAH_label <- str_replace(sceP_clusters$CHETAH_label, "Node4", "unknown")
sceP_clusters$CHETAH_label <- str_replace(sceP_clusters$CHETAH_label, "Unassigned", "unknown")
sceP_clusters$SCINA_label <- str_replace(sceP_clusters$SCINA_label, "endotelio", "unknown")
sceP_clusters$SCINA_label <- str_replace(sceP_clusters$SCINA_label, "fibroblasto", "unknown")

# Combine the 3 annotation methods results
all_res <-data.frame(
  sceP_clusters$SCINA_label,
  sceP_clusters$CHETAH_label,
  sceP_clusters$singleR_label
)

# Obtain the consensum and generate an output tsv file
...
write.table(all_res,file=paste0("processed_data/matrix_annot_P.tsv"), sep="\t", row.names = FALSE)

# Annotated SCE creation
sce.annot  <- sceP_clusters
sceP_clusters$tipo.celular  <- all_res$cell_type
sce.annot$tipo.celular  <- all_res$cell_type
saveRDS(sceP_clusters, file  <- "processed_data/sceP_clusters_filtered_annot.rds")
...
a <- ggplot(umap_data, aes(x = V1, y = V2, color = cell_type)) +
  ...
  facet_wrap(~ Group_sex,
             labeller = labeller(Group_sex = c(
               "Alzheimer's Disease_Female" = "AD_Mujer",
...
sce.annot <- sce.annot[,!(sce.annot$tipo.celular != "Excitadoras" & sce.annot$cluster40 == 1)]
sce.annot <- sce.annot[,!(sce.annot$tipo.celular != "Oligodendrocitos" & sce.annot$cluster40 == 2)]
sce.annot <- sce.annot[,!(sce.annot$tipo.celular != "Oligodendrocitos" & sce.annot$cluster40 == 3)]
sce.annot <- sce.annot[,!(sce.annot$tipo.celular != "Excitadoras" & sce.annot$cluster40 == 4)]
sce.annot <- sce.annot[,!(sce.annot$tipo.celular != "Astrocitos" & sce.annot$cluster40 == 5)]
sce.annot <- sce.annot[,!(sce.annot$tipo.celular != "OPC" & sce.annot$cluster40 == 6)]
sce.annot <- sce.annot[,!(sce.annot$tipo.celular != "Microglia" & sce.annot$cluster40 == 7)]
sce.annot <- sce.annot[,!(sce.annot$tipo.celular != "Inhibidoras" & sce.annot$cluster40 == 8)]
sce.annot <- sce.annot[,!sce.annot$cluster40 == 9] # Delete whole cluster because the cell type is not clear
...
sce.annot <- sce.annot[,!sce.annot$cluster40 == 12] # Delete whole cluster because the cell type is not clear
sce.annot <- sce.annot[,!(sce.annot$tipo.celular != "Excitadoras" & sce.annot$cluster40 == 13)]
sce.annot <- sce.annot[,!(sce.annot$tipo.celular != "Excitadoras" & sce.annot$cluster40 == 14)]
...
path2 <- "figures/7.FilteredCellTypeAnnotation/7.3.ClusteringPoisson/filtered/"
...
x <- as.data.frame(table(condicion = sce.annot$Note, cluster = sce.annot@colData[,cluster]))
...
axis_spanish_labels <- c(
  ...
  "OPC_Alzheimer's Disease_Male" = "OPC_AD_Hombre",
  "OPC_Alzheimer's Disease_Female" = "OPC_AD_Mujer",
  ...
  "Oligodendrocitos_Alzheimer's Disease_Male" = "Oligodendrocitos_AD_Hombre",
  "Oligodendrocitos_Alzheimer's Disease_Female" = "Oligodendrocitos_AD_Mujer",
  "Microglia_Control_Male" = "Microglía_Control_Hombre",
  "Microglia_Control_Female" = "Microglía_Control_Mujer",
  "Microglia_Alzheimer's Disease_Male" = "Microglía_AD_Hombre",
  "Microglia_Alzheimer's Disease_Female" = "Microglía_AD_Mujer",
  "Inhibidoras_Control_Male" = "NeuronasInhibidoras_Control_Hombre",
  "Inhibidoras_Control_Female" = "NeuronasInhibidoras_Control_Mujer",
  "Inhibidoras_Alzheimer's Disease_Male" = "NeuronasInhibidoras_AD_Hombre",
  "Inhibidoras_Alzheimer's Disease_Female" = "NeuronasInhibidoras_AD_Mujer",
  "Excitadoras_Control_Male" = "NeuronasExcitadoras_Control_Hombre",
  "Excitadoras_Control_Female" = "NeuronasExcitadoras_Control_Mujer",
  "Excitadoras_Alzheimer's Disease_Male" = "NeuronasExcitadoras_AD_Hombre",
  "Excitadoras_Alzheimer's Disease_Female" = "NeuronasExcitadoras_AD_Mujer",
  "Astrocitos_Control_Male" = "Astrocitos_Control_Hombre",
  "Astrocitos_Control_Female" = "Astrocitos_Control_Mujer",
  "Astrocitos_Alzheimer's Disease_Male" = "Astrocitos_AD_Hombre",
  "Astrocitos_Alzheimer's Disease_Female" = "Astrocitos_AD_Mujer"
)
...
saveRDS(sce.annot, file = "processed_data/sceP_clusters_filtered_annot2.rds")
...
require(openxlsx)
write.xlsx(proporciones, file = paste0("processed_data/abundancia_celularP.xlsx"),rowNames=TRUE)
