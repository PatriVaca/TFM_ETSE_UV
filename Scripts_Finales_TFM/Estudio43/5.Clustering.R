...
system("mkdir -p figures/5.Clustering/")
system("mkdir -p figures/5.Clustering/5.1.ClusteringGV")
system("mkdir -p figures/5.Clustering/5.2.ClusteringGV_dw")
system("mkdir -p figures/5.Clustering/5.3.ClusteringPoisson")
...
k_list <- list(5, 15, 20, 25, 30, 40)

##################
## ModelGeneVar ##
##################
scegv <- readRDS("processed_data/scegv.rds")

louvain_list_gv <- list()
tablesilh_list_gv <- list()
tablespure_list_gv <- list()
rmsd_list_gv <- list()
sf_list_gv <- list()
...
for (knum in k_list) {
  ## Clustering ##
  set.seed(100)
  clustering_results <- do_clustering(scegv, knum, louvain_list_gv)
  scegv <- clustering_results[[1]]
  louvain_list_gv <- clustering_results[[2]]
  
  ## Quantifying clustering behavior ##
  # Current cluster (of scegv in this case)
  clusters_k <- colData(scegv)[[paste0("cluster", knum)]]
  
  # Silhouette width
  silh <- SilhouetteWidthEvaluation(scegv, clusters_k, knum, tablesilh_list_gv)
  tablesilh_list_gv <- silh[[1]]
  silhplot <- silh[[2]]
  # Save the plot
  ggsave(paste0("./figures/5.Clustering/5.1.ClusteringGV/Silhouette_k", knum, ".jpeg", sep=""),
         plot=silhplot, width=10, height=6, units="in", device="jpeg", dpi=300)
  ggsave(paste0("./figures/5.Clustering/5.1.ClusteringGV/Silhouette_k", knum, ".svg", sep=""),
         plot=silhplot, width=10, height=6, units="in", device="svg")

  # Purity
  pure <- ClusterPurityEvaluation(scegv, clusters_k, knum, tablespure_list_gv)
  tablespure_list_gv <- pure[[1]]
  purityplot <- pure[[2]]
  # Save the plot
  ggsave(paste0("./figures/5.Clustering/5.1.ClusteringGV/Purity_k", knum, ".jpeg", sep=""),
         plot=purityplot, width=10, height=6, units="in", device="jpeg", dpi=300)
  ggsave(paste0("./figures/5.Clustering/5.1.ClusteringGV/Purity_k", knum, ".svg", sep=""),
         plot=purityplot, width=10, height=6, units="in", device="svg")

  # RMSD intra-cluster
  rmsd_results <- RMSDcluster(scegv, clusters_k, knum, rmsd_list_gv)
  rmsd_list_gv <- rmsd_results[[1]]
  rmsd_plot <- rmsd_results[[2]]
  rmsd <- rmsd_results[[3]]
  # Save the plot
  ggsave(paste0("./figures/5.Clustering/5.1.ClusteringGV/RMSD_k", knum, ".jpeg", sep=""),
         plot=rmsd_plot, width=10, height=6, units="in", device="jpeg", dpi=300)
  ggsave(paste0("./figures/5.Clustering/5.1.ClusteringGV/RMSD_k", knum, ".svg", sep=""),
         plot=rmsd_plot, width=10, height=6, units="in", device="svg")

  
  # RMSD vs Size Factors Relation
  sfresults <- RMSD_vs_SF(scegv, clusters_k, knum, sf_list_gv, rmsd)
  sf_list_gv <- sfresults[[1]]
  pointplot <- sfresults[[2]]
  # Save the plot
  ggsave(paste0("./figures/5.Clustering/5.1.ClusteringGV/RMSD_vs_SF_k", knum, ".jpeg", sep=""),
         plot=pointplot, width=10, height=6, units="in", device="jpeg", dpi=300)
  ggsave(paste0("./figures/5.Clustering/5.1.ClusteringGV/RMSD_vs_SF_k", knum, ".svg", sep=""),
         plot=pointplot, width=10, height=6, units="in", device="svg")

  folder <- "5.1.ClusteringGV"
  # PCA
  PCA_evaluation(scegv, knum, folder)

  # tSNE
  tSNE_evaluation(scegv, knum, folder)

  # UMAP
  UMAP_evaluation(scegv, knum, folder)
}
...
sceP <- readRDS("processed_data/sceP.rds")

louvain_list_P <- list()
tablesilh_list_P <- list()
tablespure_list_P <- list()
rmsd_list_P <- list()
sf_list_P <- list()
...
for (knum in k_list) {
  ## Clustering ##
  set.seed(100)
  clustering_results <- do_clustering(sceP, knum, louvain_list_P)
  sceP <- clustering_results[[1]]
  louvain_list_P <- clustering_results[[2]]
  
  ## Quantifying clustering behavior ##
  # Current cluster (of sceP in this case)
  clusters_k <- colData(sceP)[[paste0("cluster", knum)]]
  
  # Silhouette width
  silh <- SilhouetteWidthEvaluation(sceP, clusters_k, knum, tablesilh_list_P)
  tablesilh_list_P <- silh[[1]]
  silhplot <- silh[[2]]
  # Save the plot
  ggsave(paste0("./figures/5.Clustering/5.3.ClusteringPoisson/Silhouette_k", knum, ".jpeg", sep=""),
         plot=silhplot, width=10, height=6, units="in", device="jpeg", dpi=300)
  ggsave(paste0("./figures/5.Clustering/5.3.ClusteringPoisson/Silhouette_k", knum, ".svg", sep=""),
         plot=silhplot, width=10, height=6, units="in", device="svg")
  
  # Purity
  pure <- ClusterPurityEvaluation(sceP, clusters_k, knum, tablespure_list_P)
  tablespure_list_P <- pure[[1]]
  purityplot <- pure[[2]]
  # Save the plot
  ggsave(paste0("./figures/5.Clustering/5.3.ClusteringPoisson/Purity_k", knum, ".jpeg", sep=""),
         plot=purityplot, width=10, height=6, units="in", device="jpeg", dpi=300)
  ggsave(paste0("./figures/5.Clustering/5.3.ClusteringPoisson/Purity_k", knum, ".svg", sep=""),
         plot=purityplot, width=10, height=6, units="in", device="svg")
  
  # RMSD intra-cluster
  rmsd_results <- RMSDcluster(sceP, clusters_k, knum, rmsd_list_P)
  rmsd_list_P <- rmsd_results[[1]]
  rmsd_plot <- rmsd_results[[2]]
  rmsd <- rmsd_results[[3]]
  # Save the plot
  ggsave(paste0("./figures/5.Clustering/5.3.ClusteringPoisson/RMSD_k", knum, ".jpeg", sep=""),
         plot=rmsd_plot, width=10, height=6, units="in", device="jpeg", dpi=300)
  ggsave(paste0("./figures/5.Clustering/5.3.ClusteringPoisson/RMSD_k", knum, ".svg", sep=""),
         plot=rmsd_plot, width=10, height=6, units="in", device="svg")
  
  # RMSD vs Size Factors Relation
  sfresults <- RMSD_vs_SF(sceP, clusters_k, knum, sf_list_P, rmsd)
  sf_list_P <- sfresults[[1]]
  pointplot <- sfresults[[2]]
  # Save the plot
  ggsave(paste0("./figures/5.Clustering/5.3.ClusteringPoisson/RMSD_vs_SF_k", knum, ".jpeg", sep=""),
         plot=pointplot, width=10, height=6, units="in", device="jpeg", dpi=300)
  ggsave(paste0("./figures/5.Clustering/5.3.ClusteringPoisson/RMSD_vs_SF_k", knum, ".svg", sep=""),
         plot=pointplot, width=10, height=6, units="in", device="svg")
  
  folder <- "5.3.ClusteringPoisson"
  # PCA
  PCA_evaluation(sceP, knum, folder)
  
  # tSNE
  tSNE_evaluation(sceP, knum, folder)
  
  # UMAP
  UMAP_evaluation(sceP, knum, folder)
}

# ---- Save the data ------
saveRDS(scegv, "processed_data/scegv_clusters.rds")
saveRDS(scegv_dw, "processed_data/scegv_dw_clusters.rds")
saveRDS(sceP, "processed_data/sceP_clusters.rds")
