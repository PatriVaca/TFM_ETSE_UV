...
system("mkdir -p figures/6.CellTypeAnnot/6.1.ClusteringGV/6.1.2.SingleR")
system("mkdir -p figures/6.CellTypeAnnot/6.1.ClusteringGV/6.1.3.CHETAH")
...
system("mkdir -p figures/6.CellTypeAnnot/6.3.ClusteringPoisson/6.3.2.SingleR")
system("mkdir -p figures/6.CellTypeAnnot/6.3.ClusteringPoisson/6.3.3.CHETAH")
...
sce.filtgv <- scegv_clusters[rownames(scegv_clusters) %in% unlist(gm.manual.3),]
...
sce.filtP <- sceP_clusters[rownames(sceP_clusters) %in% unlist(gm.manual.3),]
...
path1_2 <- "./figures/6.CellTypeAnnot/6.1.ClusteringGV/6.1.2.SingleR/"
path1_3 <- "./figures/6.CellTypeAnnot/6.1.ClusteringGV/6.1.3.CHETAH/"
...
path3_2 <- "./figures/6.CellTypeAnnot/6.3.ClusteringPoisson/6.3.2.SingleR/"
path3_3 <- "./figures/6.CellTypeAnnot/6.3.ClusteringPoisson/6.3.3.CHETAH/"
...
resultsSCINA <- SCINA(sce.matrix.filtgv, gm.manual.3, max_iter = 100,
...
# Cluster40
set.seed(100)
plot <- plotReducedDim(scegv_clusters, dimred="TSNE120",
                       colour_by="SCINA_label", text_by="cluster40") +
...
resultsSCINA <- SCINA(sce.matrix.filtP, gm.manual.3, max_iter = 100,
                     convergence_n = 10, convergence_rate = 0.999,
                     sensitivity_cutoff = 0.9, rm_overlap=TRUE,
                     allow_unknown=TRUE, log_file='SCINA.log')
...
resultsSingleR <- SingleR(test=sce.matrix.filtgv,
                          ref=sce.downsample,
                          labels=sce.downsample$tipos_celulares,
                          assay.type.test="logcounts",
                          assay.type.ref="logcounts")

colData(scegv_clusters)$singleR_label <- resultsSingleR$labels

## TSNE
# Cluster40
set.seed(100)
plot <- plotReducedDim(scegv_clusters, dimred="TSNE120", colour_by="singleR_label", text_by="cluster40") +
    ggtitle("t-SNE perplexity = 120 y k = 40") +
    scale_color_viridis_d(name="Tipo celular") +
    labs(x = "TSNE1", y = "TSNE2")
ggsave(filename=paste0(path1_2, "TSNE120_k40.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path1_2, "TSNE120_k40.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")

## UMAP
# Cluster40
set.seed(100)
plot <- plotReducedDim(scegv_clusters, dimred="UMAP300", colour_by="singleR_label", text_by="cluster40") +
    ggtitle("UMAP n_neighbors = 300 y k = 40") +
    scale_color_viridis_d(name="Tipo celular") +
    labs(x = "UMAP1", y = "UMAP2")
ggsave(filename=paste0(path1_2, "UMAP300_k40.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path1_2, "UMAP300_k40.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")

## ScoreHeatmap (singleR function)
plot <- plotScoreHeatmap(resultsSingleR)
ggsave(filename=paste0(path1_2, "ScoreHeatmap.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path1_2, "ScoreHeatmap.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")

## Heatmap
# Cluster40
chosen_cluster <- "cluster40"
tab <- table(Assigned = scegv_clusters$singleR_label,
             Cluster = scegv_clusters[[chosen_cluster]])
plot <- pheatmap::pheatmap(log2(tab + 10),
                   cluster_rows = FALSE,
                   cluster_cols = FALSE,
                   color = colorRampPalette(c("white", "blue"))(101))
ggsave(filename=paste0(path1_2, "Heatmap_k40.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path1_2, "Heatmap_k40.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")
...
resultsSingleR <- SingleR(test=sce.matrix.filtP,
                          ref=sce.downsample,
                          labels=sce.downsample$tipos_celulares,
                          assay.type.test="logcounts",
                          assay.type.ref="logcounts")

colData(sceP_clusters)$singleR_label <- resultsSingleR$labels

## TSNE
# Cluster40
set.seed(100)
plot <- plotReducedDim(sceP_clusters, dimred="TSNE120", colour_by="singleR_label", text_by="cluster40") +
    ggtitle("t-SNE perplexity = 120 y k = 40") +
    scale_color_viridis_d(name="Tipo celular") +
    labs(x = "TSNE1", y = "TSNE2")
ggsave(filename=paste0(path3_2, "TSNE120_k40.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path3_2, "TSNE120_k40.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")

## UMAP
# Cluster40
set.seed(100)
plot <- plotReducedDim(sceP_clusters, dimred="UMAP300", colour_by="singleR_label", text_by="cluster40") +
    ggtitle("UMAP n_neighbors = 300 y k = 40") +
    scale_color_viridis_d(name="Tipo celular") +
    labs(x = "UMAP1", y = "UMAP2")
ggsave(filename=paste0(path3_2, "UMAP300_k40.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path3_2, "UMAP300_k40.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")

## ScoreHeatmap
plot <- plotScoreHeatmap(resultsSingleR)
ggsave(filename=paste0(path3_2, "ScoreHeatmap.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path3_2, "ScoreHeatmap.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")

## Heatmap
# Cluster40
chosen_cluster <- "cluster40"
tab <- table(Assigned = sceP_clusters$singleR_label,
             Cluster = sceP_clusters[[chosen_cluster]])
plot <- pheatmap::pheatmap(log2(tab + 10),
                   cluster_rows = FALSE,
                   cluster_cols = FALSE,
                   color = colorRampPalette(c("white", "blue"))(101))
ggsave(filename=paste0(path3_2, "Heatmap_k40.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path3_2, "Heatmap_k40.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")

saveRDS(scegv_clusters, "processed_data/scegv_clusters_annot2.rds")
saveRDS(scegv_dw_clusters, "processed_data/scegv_dw_clusters_annot2.rds")
saveRDS(sceP_clusters, "processed_data/sceP_clusters_annot2.rds")
...
resultsCHETAHgv <- CHETAHclassifier(input=scegv_clusters,
                        ref_cells=sce.downsample,
                        ref_ct="tipos_celulares",
                        input_c="logcounts",
                        ref_c="logcounts")

colData(scegv_clusters)$CHETAH_label <- resultsCHETAHgv$celltype_CHETAH

## TSNE
# Cluster40
set.seed(100)
plot <- plotReducedDim(scegv_clusters, dimred="TSNE120", colour_by="CHETAH_label", text_by="cluster40") +
    ggtitle("t-SNE perplexity = 120 y k = 40") +
    scale_color_viridis_d(name="Tipo celular") +
    labs(x = "TSNE1", y = "TSNE2")
ggsave(filename=paste0(path1_3, "TSNE120_k40.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path1_3, "TSNE120_k40.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")

## UMAP
# Cluster40
set.seed(100)
plot <- plotReducedDim(scegv_clusters, dimred="UMAP300", colour_by="CHETAH_label", text_by="cluster40") +
    ggtitle("UMAP n_neighbors = 300 y k = 40") +
    scale_color_viridis_d(name="Tipo celular") +
    labs(x = "UMAP1", y = "UMAP2")
ggsave(filename=paste0(path1_3, "UMAP300_k40.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path1_3, "UMAP300_k40.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")

## Heatmap
# Cluster40
chosen_cluster <- "cluster40"
tab <- table(Assigned = scegv_clusters$CHETAH_label,
             Cluster = scegv_clusters[[chosen_cluster]])
plot <- pheatmap::pheatmap(log2(tab + 10),
                   cluster_rows = FALSE,
                   cluster_cols = FALSE,
                   color = colorRampPalette(c("white", "blue"))(101))
ggsave(filename=paste0(path1_3, "Heatmap_k40.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path1_3, "Heatmap_k40.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")
...
resultsCHETAHP <- CHETAHclassifier(input=sceP_clusters,
                                  ref_cells=sce.downsample,
                                  ref_ct="tipos_celulares",
                                  input_c="logcounts",
                                  ref_c="logcounts")

colData(sceP_clusters)$CHETAH_label <- resultsCHETAHP$celltype_CHETAH

## TSNE
# Cluster40
set.seed(100)
plot <- plotReducedDim(sceP_clusters, dimred="TSNE120", colour_by="CHETAH_label", text_by="cluster40") +
    ggtitle("t-SNE perplexity = 120 y k = 40") +
    scale_color_viridis_d(name="Tipo celular") +
    labs(x = "TSNE1", y = "TSNE2")
ggsave(filename=paste0(path3_3, "TSNE120_k40.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path3_3, "TSNE120_k40.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")

## UMAP
# Cluster40
set.seed(100)
plot <- plotReducedDim(sceP_clusters, dimred="UMAP300", colour_by="CHETAH_label", text_by="cluster40") +
    ggtitle("UMAP n_neighbors = 300 y k = 40") +
    scale_color_viridis_d(name="Tipo celular") +
    labs(x = "UMAP1", y = "UMAP2")
ggsave(filename=paste0(path3_3, "UMAP300_k40.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path3_3, "UMAP300_k40.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")

## Heatmap
# Cluster40
chosen_cluster <- "cluster40"
tab <- table(Assigned = sceP_clusters$CHETAH_label,
             Cluster = sceP_clusters[[chosen_cluster]])
plot <- pheatmap::pheatmap(log2(tab + 10),
                   cluster_rows = FALSE,
                   cluster_cols = FALSE,
                   color = colorRampPalette(c("white", "blue"))(101))
ggsave(filename=paste0(path3_3, "Heatmap_k40.jpeg", sep=""),
       plot=plot, width=10, height=6, units="in", device="jpeg", dpi=300)
ggsave(filename=paste0(path3_3, "Heatmap_k40.svg", sep=""),
       plot=plot, width=10, height=6, units="in", device="svg")

#---------------
saveRDS(scegv_clusters, "processed_data/scegv_clusters_annot3.rds")
saveRDS(scegv_dw_clusters, "processed_data/scegv_dw_clusters_annot3.rds")
saveRDS(sceP_clusters, "processed_data/sceP_clusters_annot3.rds")