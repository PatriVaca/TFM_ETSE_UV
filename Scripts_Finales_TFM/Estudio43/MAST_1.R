...
# Remove the apostrophe to be able to run the later lrTest
colData(sce.annot)$Group_sex <- gsub("'s ", "s_", colData(sce.annot)$Group_sex)
...
for (i in 1:length(cell_types)){
  ...
  # Establish the ENSEMBLIDs as rownames for further analysis
  ...
  zlmCond <- zlm(~0+Group_sex+cngeneson+Sample, sce.assay_cells, exprs_values = "log2tpm", method = "bayesglm", ebayes = TRUE, parallel = TRUE)
  # Save the model
