...
# ENSEMBLID ANNOTATION
symbols <- rownames(sce.annot)

# Map gene symbols to ENSEMBL ids
ensemblids <- mapIds(
  org.Hs.eg.db,
  keys = rownames(sce.annot),
  keytype = "SYMBOL",
  column = "ENSEMBL",
  multiVals = "first" # In case of multiple ENSEMBL ids per gene symbol, keep the first one
)

# Add the ENSEMBL ids to the sce
rowData(sce.annot)[,"ENSEMBL"] <- ensemblids
# Add the gene symbols to the rowData too
rowData(sce.annot)[,"SYMBOL"] <- rownames(sce.annot)

# Check if there is any Gene Symbol or ENSEMBLID = NA
table(is.na(rownames(sce.annot)))
table(is.na(rowData(sce.annot)$ENSEMBL))

# Filter out genes with NA as Gene Symbol
sce.annot <- sce.annot[!(is.na(rownames(sce.annot))), ]

# Filter out genes with NA as ENSEMBLID
sce.annot <- sce.annot[!is.na(rowData(sce.annot)$ENSEMBL), ]

# sum(duplicated(rowData(sce.annot)$ENSEMBL))
...
sce.annot <- sce.annot[!duplicated(rowData(sce.annot)$ENSEMBL), ]
sce.annot <- sce.annot[!duplicated(rownames(sce.annot)), ]

saveRDS(sce.annot, "processed_data/scegv_dw_clusters_filtered_annot2.rds")
...
#--------- Select cell type ---------#
# Keep just the cell types that have a considerable number
cell_types <- c("Astrocitos", "Excitadoras", "Oligodendrocitos", "Microglia")
...
for (i in 1:length(cell_types)){
  ...
  sce.dge <- sce.annot[,sce.annot$tipo.celular == cell_type]
  ...
  rowData$ENSEMBL <- unlist(sapply(strsplit(rowData$ENSEMBL, ".", fixed = TRUE),
                                   `[`, 1, simplify=FALSE)) # Filter ENSEMBLids
  
  # Set the ENSEMBLIDs as rownames for further analysis
  rownames(sce.dge) <- rowData(sce.dge)$ENSEMBL 
  ...
  # Remove duplicates and possible NA values
  ...
  zlmCond <- zlm(~0+Group_sex+cngeneson+individualID, sce.assay_cells, exprs_values = "log2tpm", method = "bayesglm", ebayes = TRUE, parallel = TRUE)
  # Save the model
