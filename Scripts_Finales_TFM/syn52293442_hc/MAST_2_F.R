...
for (i in 1:length(cell_types)){
  ...
  # Test hypothesis
  zlmCond_cells <- lrTest(zlmCond, Hypothesis("Group_sexAD_Female - Group_sexControl_Female"))
  ...
  # Calculate LogFCs
  LFC.sF <- LFC.standard(data = sce.dge, results = results, case = "AD_Female", control = "Control_Female")
  
  LFC.mF <- LFC.MAST(zlm = zlmCond, control = "Group_sexControl_Female", case = "Group_sexAD_Female", results = results)
