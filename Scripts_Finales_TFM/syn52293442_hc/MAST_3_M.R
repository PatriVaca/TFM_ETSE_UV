...
for (i in 1:length(cell_types)){
  ...
  zlmCond_cells_M <- lrTest(zlmCond, Hypothesis("Group_sexAD_Male - Group_sexControl_Male"))
  ...
  # Calculate the LFCs
  LFC.sM <- LFC.standard(data = sce.dge, results = results, case = "AD_Male", control = "Control_Male")
  
  LFC.mM <- LFC.MAST(zlm = zlmCond, control = "Group_sexControl_Male", case = "Group_sexAD_Male", results = results)
